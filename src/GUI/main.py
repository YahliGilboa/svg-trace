...
from PySide6.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout,
                               QLabel, QPushButton, QFileDialog, QScrollArea, QListWidget,
                               QListWidgetItem)

from color_model import ColorModel
from src.calculate_dominant_colors import calculate_main_image_colors
from src.svg_conversion_logic import convert_img_to_svg
...
class ColorItemWidget(QWidget):
    ...
    def __init__(self, index, color_data, parent=None):
        ...
        self.color_circle.setStyleSheet(
            f"background-color: rgb({r},{g},{b}); border-radius: 12px; border: 1px solid #ccc;")
        ...
        if color_data.get('excluded', False):
            ...
            self.color_circle.setStyleSheet(
                f"background-color: rgba({r},{g},{b}, 100); border-radius: 12px; border: 1px dashed #888;")

        layout.addWidget(self.info_label)
        layout.addStretch()

        # Exclude toggle button
        exclude_text = "Include in trace" if color_data.get('excluded', False) else "Exclude from trace"
        ...
        self.delete_btn.setStyleSheet(
            "QPushButton { color: red; font-weight: bold; border: none; } QPushButton:hover { background-color: #eee; }")
...
class ColorPickerApp(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("trace svg from image")
    ...
    def export_as_svg(self):
        ...
        color_pallete = np.array([list(color['rgb']) for color in self.color_model.colors])
        excluded_colors = np.array([list(color['rgb']) for color in self.color_model.colors if color['excluded']])

        convert_img_to_svg(
            img_np_array=self.np_array_image,
